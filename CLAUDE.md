...
代码位于 `代码/` 子目录，各模块独立目录，`<parent>` 通过本地 Maven 仓库引用父 POM（`relativePath` 为空）。仓库根目录另含**前端参考源码**（非后端构建依赖、不进 Maven 反应堆）：`hengde-volunteer-admin-mock/`（后台 mock 控制台静态页 `index.html`+`assets/`，对照 `/a` 接口的可视化原型，与 `文档/v1/后台管理页面设计prompt.md` 配套）、`hengde-volunteer-miniprogram/`（微信小程序源码：`app.json`/`pages`/`utils`/`custom-tab-bar` 等，无 `node_modules`）；原始下载 zip 已被 `.gitignore` 忽略（`*.zip`，解压目录已入库）。
...
- `文档/v1/后台管理页面设计prompt.md` — **后台管理前端设计 prompt**：枚举全部已实现 `/a` 接口/按钮/可视化，供据以实现后台控制台；与仓库根 `hengde-volunteer-admin-mock/`（mock 控制台原型）配套。
