...
- **已接**：登录 + `GET /a/auth/me` 真实权限码驱动菜单/按钮（会话恢复 + 退出 + 401 回登录）；**公示**（轮播图/公告/文件，CRUD+上下架+排序+可下载，图片选图→canvas 裁剪上传）；**活动**（列表/详情/发布/编辑/删除/复制、周期 `/recurring`、历史 `/historical`、详情留言列表与下架、高德地图选点[`window.__AMAP_KEY__`+安全密钥占位，无 key 回退示意]；多时间段活动暂禁单 slot 表单编辑）；**活动发布审核 / 报名管理 / 服务记录·积分·考勤变更·补录审核 / 组织(小组·分队) / 子账号·权限·志愿者标记授权**（M2 全部完成）；**志愿者管理**（M3，`/a/user/volunteers`：列表/详情/修改[仅超管]/停用·恢复/删除/导出；状态约定 `0正常/1禁用/2注销` 已与 `MAPS.acct` 翻正对齐、注销态视终态不提供前端反注销；隐藏重置密码按钮；菜单 `user:list` 显隐、操作按 `user:status`/`user:delete`/`user:export` 及超管显隐；mock `data.js` 权限码同步真实 `user:*`）；**概览数据看板**（M4，`page-overview.jsx`：头部 6 张统计卡拉 `GET /a/data/dashboard`；7 张待办卡走各权限受控端点 `size=1` 取 total——仅对当前账号<b>有该端点权限</b>的项发请求防 403，含新全局报名 `GET /a/activity/enrollments?status=0`）。
- **待接页面**：**全部业务页已接真实接口（M1–M4 完成）**。剩 nginx 分离部署 + 生产硬化（M5：React/Babel 现走 unpkg CDN，离线/提速须本地化或预编译；`data.js` 现仅余 `HD.hasPerm` 待清）。
