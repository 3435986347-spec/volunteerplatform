...
仓库根 `volunteer-platform-back/`（无构建、浏览器内 Babel 的 React 后台控制台）正从 `HD.*` mock **逐页接入真实 `/a` 接口**。详细进度见用户记忆 `project_admin_frontend_integration`。
...
- **已接**：登录 + `GET /a/auth/me` 真实权限码驱动菜单/按钮（会话恢复 + 退出 + 401 回登录）；**公示**（轮播图/公告/文件，CRUD+上下架+排序+可下载，图片选图→canvas 裁剪上传）；**活动**（列表/详情/发布/编辑/删除/复制、周期 `/recurring`、历史 `/historical`、详情留言列表与下架、高德地图选点[`window.__AMAP_KEY__`+安全密钥占位，无 key 回退示意]；多时间段活动暂禁单 slot 表单编辑）。
- **待接页面**：活动发布审核 / 报名管理 / 服务记录与积分·考勤变更·补录审核 / 组织（小组·分队）/ 子账号与权限。
- **两页缺后端模块（已决策待补建）**：志愿者管理 `/a/user/volunteers`（`user` 模块）、数据看板 `/a/data/dashboard`（`data` 模块）+ 概览待办用的全局 `GET /a/activity/enrollments?status=`。
