...
- **已接**：登录 + `GET /a/auth/me` 真实权限码驱动菜单/按钮（会话恢复 + 退出 + 401 回登录）；**公示**（轮播图/公告/文件，CRUD+上下架+排序+可下载，图片选图→canvas 裁剪上传）；**活动**（列表/详情/发布/编辑/删除/复制、周期 `/recurring`、历史 `/historical`、详情留言列表与下架、高德地图选点[`window.__AMAP_KEY__`+安全密钥占位，无 key 回退示意]；多时间段活动暂禁单 slot 表单编辑）；**活动发布审核 / 报名管理 / 服务记录·积分·考勤变更·补录审核 / 组织(小组·分队) / 子账号·权限·志愿者标记授权**（M2 全部完成）；**志愿者管理**（M3，`/a/user/volunteers`：列表/详情/修改[仅超管]/停用·恢复/删除/导出；状态约定 `0正常/1禁用/2注销` 已与 `MAPS.acct` 翻正对齐、注销态视终态不提供前端反注销；隐藏重置密码按钮；菜单 `user:list` 显隐、操作按 `user:status`/`user:delete`/`user:export` 及超管显隐；mock `data.js` 权限码同步真实 `user:*`）。
- **待接页面**：概览数据看板（M4，待 `data` 模块）→ 之后 nginx 分离部署 + 生产硬化（M5：React/Babel 现走 unpkg CDN，离线/提速须本地化或预编译）。
- **缺后端模块**：志愿者管理 `user` 模块**已补建完成**（见上「已完成」表）；剩 数据看板 `/a/data/dashboard`（`data` 模块未建）+ 概览待办用的全局 `GET /a/activity/enrollments?status=`（M4 一并补）。
...
- `data`：首页数据看板/数据展示（`/v/home` 现为占位）、投诉建议（默认到监察部）。**注**：后台「志愿者管理」所需的 `user` 模块（`/a/user/volunteers`）已随后台前端补建完成（见「已完成」表），不在本段 V1 领域顺序内。
