...
| 模块 | 路径 | 说明 |
|---|---|---|
| `hengde-volunteer-common` | `代码/hengde-volunteer-common` | **已完成**：返回体/异常/错误码（`Result`/`BusinessException`/`ResultCode`）、基础实体与自动填充（`BaseEntity`/`MyMetaObjectHandler`）、Redis（`RedisConfig`/`RedisUtil`）、加解密（`CryptoUtil`：AES-GCM + HMAC，`SecurityProperties` 持密钥，dev 默认值生产须覆盖）、密码（`PasswordUtil`）、JWT（`JwtUtil`，备用）、常量（`UserStatus`/`Gender`/`Grade`/`CommonConstants`）、短信（`SmsProperties`/`SmsService`/`VerifyCodeService`[防滥用：同号跨场景日限量+来源 IP 时/日限量（INCR 原子先计数后放行，IP 由控制器经 `IpUtil` 传入、缺省跳过）、同码校验错满 `verifyMaxAttempts` 次作废]/`SmsScene`）、客户端 IP 提取（`utils/IpUtil`：X-Real-IP→XFF 第一段→remoteAddr——优先 X-Real-IP 因 nginx 覆盖式写入不可伪造，XFF 追加式首段客户端可控[nginx.conf 已同步改覆盖式 `$remote_addr`]；仅作限流弱信号不作鉴权）、对象存储（`OssProperties`/`FileStorageService`/`AliyunOssFileStorageService`/`FileValidator`）、Excel（`ExcelUtil`）、二维码（`QrCodeUtil`：ZXing 编码 + `data:image/png;base64,...` 出参，供前端 `<image>` 直接展示，当前用于活动签到二维码；引 zxing core+javase）、分页（`PageQuery`/`PageResult`）、分布式锁助手（`lock/DistributedLockSupport`：静态工具无 bean/DI[仿 `PasswordUtil`]，`runLocked`/`runLockedMany` 集中「升序去重加锁→反序释放→`isHeldByCurrentThread` 才解→watchdog 续期、tryLock 5s 超时」死锁安全纪律，调用方传入自注入的 `RedissonClient`；common 仅引 redisson **核心库**[API]不引 starter，故 common 上下文不建 `RedissonClient`、不逼其测试连 Redis）、Testcontainers 测试基座（`TestcontainersConfig`=MySQL、`RedisTestcontainersConfig`=Redis，均 test-jar 暴露）、全局搜索统一出参（`search/SearchItemVO`：type/id/title/summary/imageUrl，跨领域聚合用）、Flyway 迁移脚本与依赖（`db/migration/`，全项目唯一序列，当前到 **V23**：V1 init、V2 RBAC、V3/V4 activity、V5 organization 小组/分队/架构、V6 publicity、V7 group admin/history、V8 activity 发布团队/联系人/分角色报名开放时间、V9 group_member「一人一组」唯一约束、V10 activity 签到/时长/积分闭环+现场负责人（activity 加经纬度/半径/run_status/总结、新建 activity_leader/activity_attendance/activity_violation、6 权限点）、V11 volunteer 加 manager_flag「管理团队」标记（接通积分 ×1.2 倍率）、V12 新增权限点 `org:manager-flag`（管理团队标记后台手动开关）、V13 volunteer 加 `manager_flag_by`/`manager_flag_time` 记标记操作人与时间、V14 新建 `activity_attendance_change`（考勤/积分变更二次审核）、V15 新建 `activity_message`（活动留言，第3批·PR1）、V16 activity 加 `is_historical` + 新建 `activity_backfill` + 2 权限点 `activity:backfill`/`backfill-audit`（历史活动+补录，第3批·PR3）、V17 volunteer 加 `signed_agreement_version`（注册记录所签协议版本，协议阅读+手写签名小批）、V18 志愿者端 RBAC：新建 `volunteer_permission`（志愿者↔权限点，镜像 admin_permission）+ `permission` 加 `volunteer_grantable` 白名单列、活动域子集（除 `activity:menu`）置可授权（把权限点扩到志愿者域；消费方如 `POST /v/activity/activities` 发布活动[PR2 已落地]）、V19 activity 加 `publish_reject_reason`/`publish_review_by`/`publish_review_time` + 1 权限点 `activity:publish-audit`（活动发布审核：小程序提交的活动落待审 status=4 须后台审核才上线，status 扩 4待审核发布/5发布被驳回，后台直发不审）、V20 volunteer 加 `password`（手机号+密码登录密文，无重复建 uk_phone_hash）、V21 volunteer 加 `nick_name` + 唯一索引 `uk_nick_name`（昵称志愿者可自助改、全局唯一去重）、V22 activity 加 `service_guarantees`（活动「服务保障」12 选 N，逗号分隔 key 存储、详情页红/灰渲染，对齐 activity 的 `ServiceGuarantee` 常量与小程序 `utils/service-guarantees.js`）、V23 新建 `manager_application`（报名管理团队申请：志愿者在小程序提交问卷/简历[reason 申请理由必填/experience 相关经历/expect_department 期望部门]，后台审核通过即置 `volunteer.manager_flag=1`，status 0待审/1通过/2驳回 + reject_reason；复用既有 `org:manager-flag` 权限点不新增；索引 idx_volunteer_status/idx_volunteer_apply_time/idx_status_apply_time）） |
| `hengde-volunteer-api` | `代码/hengde-volunteer-api` | **全局配置层已完成**：`SaTokenConfigure`、`WebMvcConfig`（CORS + **MVC 消息转换器前置**：`extendMessageConverters` 把基于 `JacksonConfig` ObjectMapper 的 Jackson-2 `MappingJackson2HttpMessageConverter` 插到 converters 首位，接管 HTTP 出入参序列化）、`JacksonConfig`（注册 `JavaTimeModule`——手建 ObjectMapper 不会自动注册 jsr310，否则 VO 的 LocalDateTime 运行期序列化抛异常；LocalDateTime→`yyyy-MM-dd HH:mm:ss`、Long→字符串防 JS 丢精度；**Boot 4 / Spring 7 的 MVC 默认走 Jackson 3（`tools.jackson`），这个 `@Primary` 的 Jackson-2 ObjectMapper 在 MVC 层本不生效**——曾致所有接口响应时间带 ISO `T`（如「报名截止：2026-06-10T09:00:00」）、Long 也不转字符串；现由上面 WebMvcConfig 前置 Jackson-2 转换器统一接管出参；**入参侧 LocalDateTime 改宽松反序列化**[`LenientLocalDateTimeDeserializer`：先按固定 `yyyy-MM-dd HH:mm:ss` 解析、失败回退 `DateTimeFormatter.ISO_LOCAL_DATE_TIME`]，同时兼容后台 datetime-local 送的 ISO `T` 与小程序送的空格格式（出参恒为空格格式）；配 `JacksonConfigTest` 烟囱测试 + `JacksonMvcFormatTest` 锁「出参无 T/Long 为字符串、双格式入参均可、转换器确用本 mapper 前置」3 例）、`MybatisPlusConfig`（含分页拦截器 `PaginationInnerInterceptor`，**仅此处装配**，领域模块测试上下文没有）、`AsyncConfig`、`SpringDocConfig`（Knife4j，分组 volunteer/admin/enterprise）、`GlobalExceptionHandler`、`TraceFilter`、`HomeAggregateController`（占位）、`SearchController`（**已实现**：`/v/search` 全局搜索，聚合 activity/publicity/organization 四类——活动+公告+小组+分队，各领域 `countSearch`+`search(offset,limit)` 跨块精确分页，合并成单一信息流 `PageResult<SearchItemVO>`）、`FileUploadController`（**通用上传** `POST /a/files/upload`：multipart `file`+**必传** `dir`，**按 dir 双门槛**——权限[banner→pub:banner/announcement→pub:announcement/file→pub:file/activity→activity:publish或edit/summary→activity:manage，经 `StpAdminUtil.STP_LOGIC.checkPermission`]+ 类型[图片目录仅 `FileValidator.IMAGE_EXTENSIONS`，仅 file 目录收文档]，未知 dir 拒；调 common `FileStorageService` 返 `{url,name,size}`，业务表只存 url；OSS 未配置回占位 URL 便于联调）、`VolunteerFileUploadController`（**志愿者端上传** `POST /v/files/upload`：给「管理团队」志愿者在小程序发活动传封面用；与 `/a/files` 分开因小程序持志愿者 token 过不了 `/a/**`，权限走默认 login 域吃志愿者 RBAC `@SaCheckPermission(activity:publish)`，**仅放 `dir=activity` 限图片**，其它目录拒）、`SuperAdminInitializer`（启动预置超管）。迁移脚本在 common；`RedissonClient` 由 `redisson-spring-boot-starter` 自动从 `spring.data.redis` 装配 |
| `hengde-volunteer-auth` | `代码/hengde-volunteer-auth` | **已完成**：志愿者端（微信登录/发注册验证码/实名注册/企业微信群校验/退出，默认 `StpUtil`，loginId=`volunteer.id`；**手机号体系登录（V20）**——账号=手机号：`POST /v/auth/login/sms`[手机号+验证码登录，scene=login，**陌生号自动建游客账号**：openid 合成 `p:`+phoneHash 前 62 位满足 NOT NULL UNIQUE≤64，建号竞态靠 `uk_phone_hash` 唯一索引+`DuplicateKeyException` 回查兜底（auth 无 Redisson 不用分布式锁）]、`POST /v/auth/login/password`[手机号+密码登录，BCrypt，接 `LoginProtectionService` 防爆破（志愿者独立前缀、key 用 phoneHash）]、`PUT /v/auth/password`[设/改密，需登录，首次原密码可空、已有须校验]、`PUT /v/auth/password/reset`[忘记密码，scene=volunteer-password-reset]；`volunteer` 加 `password` 列（V20，无重复建 uk_phone_hash）；`SmsScene` 加 `LOGIN`/`VOLUNTEER_PASSWORD_RESET`，`sendSmsCode` 按 {register,login,volunteer-password-reset,change-phone} 白名单发码；`register` 落库前显式查 phoneHash 防串号；**注册手机号锁定=登录手机号（方案A）**——已绑手机号的账号（手机号验证码登录建号）注册手机号须与登录号一致[不一致拒]，且登录时已验过该号故**注册不再重复发 REGISTER 验证码**（`phoneAlreadyVerified` 跳过校验），小程序注册页手机号置灰带出不可改、隐藏验证码行；微信登录等未绑手机号账号仍走 REGISTER 验证码绑定；不存在游客号与实名号并存——游客→志愿者同账号原地升级（需求 R49「切换账号」指志愿者/管理团队/企业**角色身份**切换，非游客↔实名）；注册 `politicalStatus`/`grade` 入参是 Integer code 非中文 label（小程序按 picker 下标+1 映射后提交，否则 Jackson 反序列化 400）；登录均拒 status≠NORMAL，失败文案不泄露存在性；微信端点保留但小程序默认改走手机号体系（同人双身份不合并，已知限制）；**开发登录** `POST /v/auth/login/dev`——`AuthProperties.devLoginEnabled` 开关[默认 false、dev profile 置 true、生产由 `ProductionConfigGuard` fail-fast 拒绝]，跳过微信 code 换 openid，按 `key` 找/建志愿者[openid=`dev:{key}`]发 token，`registered=true` 直接造已实名身份供前端联调）；**协议阅读+手写签名**（`GET /v/auth/agreement` 公开返回协议正文+版本[`AuthProperties.agreementVersion/Text` 可配]；`RegisterDTO.signatureUrl` 注册必填[`@NotBlank`]存 `volunteer.signature_url`；注册时把服务端当前协议版本写 `signed_agreement_version` 留痕，V17）；管理端（账号登录/找回密码，`StpAdminUtil` 独立 `StpLogic`，与志愿者端隔离；**登录防爆破** `LoginProtectionService`——账号维度连续失败 `loginMaxFailures` 次锁 `loginLockSeconds`[默认 5 次/15 分钟，达上限重置满时长，成功清零]+IP 维度喷洒上限 `loginIpMaxFailures`[成功不清、窗口自然过期]，Redis INCR 计数，禁用账号不计失败；测试 `AdminLoginProtectionTest`/`VerifyCodeProtectionTest` 在 auth 跑[common 无测试启动类]，MySQL+Redis 双容器）；志愿者 PII（身份证/手机号）AES-GCM 密文 + HMAC 哈希列；`VolunteerQueryService` 跨模块只读出参（`VolunteerProfileView` 资格档案、`VolunteerDisplayView` 展示信息含解密手机号、`listNamesByIds` 只取姓名不解密手机号[公开展示如活动留言用]、`findForBackfill` 按身份证/手机号 HMAC 精确匹配唯一志愿者[活动补录定位，命中多人或姓名不符拒、身份证+手机号同传须命中同一人，回 `VolunteerBackfillView` 不含 PII]、`isManager` 管理团队判定）；`VolunteerAdminService`（管理端写，与只读 query 分开：`setManagerFlag(id,flag,operatorId)` 设/取消「管理团队」标记——flag 硬校验仅 0/1[非法抛异常不静默取消]、设为 1 要求已实名、取消 0 放行[可清游客脏标记]、幂等、落 `manager_flag_by`/`time` 审计；鉴权由 organization 控制器负责） |
| `hengde-volunteer-organization` | `代码/hengde-volunteer-organization` | **已完成**。子账号/RBAC（`com.hengde.organization` 包）：`permission` 种子（V2 23 个 + V4 `activity:enroll-view` + V10 活动 6 个 + V12 `org:manager-flag` + V16 活动 2 个 `activity:backfill`/`backfill-audit` + V19 `activity:publish-audit`，共 34；**V18** 不增点、给 `permission` 加 `volunteer_grantable` 列标记「可授权给志愿者」的点[活动域子集除 `activity:menu`]）、`AdminStpInterface`（`@SaCheckPermission` 权限数据源，超管走 `*` 通配；**V18 起** `loginType=login` 志愿者分支按 `volunteer_permission` 供权限——经 `VolunteerQueryService.isActiveManager` 挡停用/降级，仅活跃且 `manager_flag=1` 者凭授权点在 `/v` 鉴权，取消标记即时失效）、子账号管理与权限分配（`assignPermissions` 仅超管，service 层防自助提权；`user:edit`/`org:perm-assign` 写死仅超管、不入权限表）。小组/分队（`com.hengde.organization.biz` 包，V5+V7 表）：`GroupService`（小组发起/审批加入/退出/转移组长/解散[V7 起 dissolve_time/reason/by 独立字段，与 reject_reason 解耦；同步清成员防卡「一人一组」]/批量导入/V7 起组长 setAdmin·revokeAdmin[≤3 人]+管理员可与组长一同 approveMember/rejectMember/removeMember；`joinApplications`「待审核加入申请列表」[仅组长/管理员可见，返回 PENDING 加入行的 memberId/申请人/申请时间，role=普通成员过滤掉建组时的 PENDING 组长行——补上「approve/reject 需 memberId 但无接口列待审」的 API 层断链]；审批写 audit_by；`transferLeader`/建组通过写入 `volunteer_group_leader_history`；V9 `approveMember`/`rejectMember` 改 CAS 防并发覆盖；`approveCreate`/`rejectCreate` 也改 CAS；V9 DB 唯一约束 `uk_active_volunteer(active_volunteer_lock)` 兜底「一人一组」；`importGroups` 仅建 ACTIVE 小组[显式非 ACTIVE 行直接拒绝——导入 PENDING 不插组长成员行会绕开「一人一组」且让 `approveCreate` 找不到待生效组长行成死组]，每行 `ensureNoActiveGroup` + 必插 ACTIVE 组长成员行 + `DuplicateKeyException` 友好降级；**导入视同审批**：写 `approved_time`/`approved_by` + 组长成员 `audit_by` + 补一条 `OP_TYPE_INITIAL` 组长历史，审计链与 `approveCreate` 对齐；解析与落库拆开为 `importGroups(MultipartFile,adminId)`→`importGroupRows(List,adminId)`，后者可脱离 Excel 单测；小组 create/join 用 Redisson「志愿者维度」锁——委托 common `DistributedLockSupport.runLocked`（key 前缀 `lock:group:volunteer:`）防同人多组竞态；`list`/`joinApplications` 去 N+1[组长名 `listNamesByIds`、ACTIVE 成员数 `group_by` 聚合各一次批量取]）、`GroupQueryService`（V7 跨模块只读：`requireSameActiveGroup` 供 activity 代报名校验同小组）、`SquadService`（分队增改删[`delete` 走 `@TableLogic` 逻辑删非物理删]、`list`/`applications` 去 N+1[分队名/志愿者名/成员数批量取]、申请加入/审批[CAS 条件更新 + 人数上限预校验 + `squad_id is null` 条件归属]、归属差异视图[未归属仅看负责人；已归属看同分队成员，字段按 `visible_fields` 收敛]、停用分队志愿者端不可达 `requireEnabledSquad`）、`StructureService`（组织架构树）、`SquadQueryService`（跨模块只读：`listNamesByIds` squadId→分队名批量，供 user「志愿者管理」展示归属分队）。`GroupQueryService` V18 起加 `listActiveGroupNamesByVolunteerIds`（志愿者→所在 ACTIVE 小组名批量，同供 user 域）。志愿者管理团队标记（V12 权限点 `org:manager-flag`）：`OrganizationVolunteerController` 的 `PUT /a/organization/volunteers/{id}/manager-flag` 调 auth `VolunteerAdminService.setManagerFlag` 手动开关 `volunteer.manager_flag`（DTO flag 0/1 校验 + service 层硬校验兜底；操作人取 `StpAdminUtil.getLoginIdAsLong()` 落审计；「报名管理团队」问卷审批已落地[V23，见本行末]，复用同一标记通道）。**志愿者端 RBAC 基建（V18，把权限点扩到志愿者域；打通「授权→志愿者 token 带权限码→`@SaCheckPermission` 在 `/v` 生效」链路。消费这些码的 `/v` 动作：`POST /v/activity/activities` 发布活动[`activity:publish`，PR2 已落地，见 activity 行]；现场负责人管理仍走 `/v/activity/managed-activities` 逐活动 `requireVolunteerLeader`）**：`VolunteerPermission`/`VolunteerPermissionMapper`（`selectCodesByVolunteerId` 额外按 `volunteer_grantable=1` 过滤兜底防脏行外泄）、`VolunteerPermissionService`（`assignPermissionsBy` 仅超管 + 目标须 `manager_flag=1`[防误授普通/游客态志愿者发活动] + 白名单校验[非活动域点拒] + 全量替换[传空 permissionIds=清空，免 manager 门槛、便于降级清理 stale 授权]；`myCodes`[同 `isActiveManager` 口径，降级即失效]/`listAssigned`；显式操作人入口供测试，仿 `joinApplicationsBy`）、`PermissionService.listGrantableToVolunteer`；控制器 `OrganizationVolunteerController` 加 `GET/PUT /a/organization/volunteers/{id}/permissions`（PUT 仅超管）、`PermissionController` 加 `GET /a/organization/permissions/volunteer-grantable`、新 `MyPermissionController` 的 `GET /v/organization/my-permissions`[前端据此显示/隐藏入口，仅 UX，后端各动作仍 `@SaCheckPermission` 兜底]。**后台前端基建接口（补齐设计 prompt 的「0 号前置」）**：`AdminMeService`/`AdminMeController` 的 `GET /a/auth/me`（当前管理员资料+权限码 `AdminMeVO{adminId,username,realName,department,superAdmin,permissionCodes}`，超管 codes=`["*"]`；落 organization 因要同读 auth 的 AdminUser 与本域权限表，auth 不能反依赖；登录态即可不挂权限点）；`SquadService.applications(PageQuery,status)` + `AdminSquadController` 的 `GET /a/organization/squads/applications`（**全局**待审分队加入申请，不按分队 id，默认 status=0 可覆盖，每行带 squadName，供概览待办/统一审批，`org:squad-audit`）；`SquadApplicationVO` 加 `squadName`。**报名管理团队（问卷+审核 → manager_flag，V23）**（`com.hengde.organization.biz`，仿 `SquadApplication` 范式；志愿者在小程序提交「报名管理团队」问卷/简历→后台审核通过即标记为管理团队，承接上面预告的「同一标记通道」复用）：`ManagerApplication`(entity)/`ManagerApplicationVO`(带 volunteerName)/`ManagerApplicationMapper`/`ManagerApplyDTO`(reason `@NotBlank @Size(500)` + experience/expectDepartment 可空)/`RejectManagerApplicationDTO`(reason `@Size(512)`)；`ManagerApplicationService`：`apply`（**按 volunteerId 上 Redisson 锁**[复用 common `DistributedLockSupport.runLocked`，key 前缀 `lock:manager-apply:volunteer:`，防双击/弱网重放插两条待审]+ `transactionTemplate`；锁内仅**正常且已实名**账号可申请[`VolunteerQueryService.isActive` + `getFlagInfo().registered` 兜底 null]、已有 status=0 待审拒、已 `manager_flag=1` 拒；落 status=0）、`myApplication`（本人最近一条 `order by apply_time desc,id desc limit 1`）、`list`（默认 status=0 待审，`listNamesByIds` 批量带姓名）、`approve`（`@Transactional`，**顺序钉死**：`requirePending` → 校验申请人仍 active+registered[提交后被禁用/注销则抛错、申请保持待审] → `volunteerAdminService.setManagerFlag(volunteerId,1,adminId)` → **最后 CAS** `eq(status,0) set status=1`，affected!=1 抛错整事务回滚含 manager_flag；**仅置 manager_flag=1、不自动授任何权限点**——具体功能权限仍由超管在授权页单独给）、`reject`（CAS 0→2 + reject_reason，service ≤512 兜底）；active+registered 校验收成私有 `requireActiveRegisteredVolunteer`。控制器 `MyManagerApplicationController` `/v/organization/manager-applications`（`POST` apply[StpUtil 取志愿者 id] / `GET /mine`）、`AdminManagerApplicationController` `/a/organization/manager-applications`（`GET ?status=` / `POST /{id}/approve` / `POST /{id}/reject`，均 `@SaCheckPermission(org:manager-flag, type=admin)` + StpAdminUtil 操作人）。测试 `ManagerApplicationServiceTest` 11/11（Testcontainers MySQL+Redis） |
...
- **已接**：登录 + `GET /a/auth/me` 真实权限码驱动菜单/按钮（会话恢复 + 退出 + 401 回登录）；**公示**（轮播图/公告/文件，CRUD+上下架+排序+可下载，图片选图→canvas 裁剪上传）；**活动**（列表/详情/发布/编辑/删除/复制、周期 `/recurring`、历史 `/historical`、详情留言列表与下架、高德地图选点[`window.__AMAP_KEY__`+安全密钥占位，无 key 回退示意]；**多场次发布/编辑**——`SlotsEditor` 可增删场次卡片[项目名称/日期/起止/需求人数]，活动整体起止由各场次最早~最晚派生后随 `slots[]` 提交，旧「多时间段暂禁编辑」限制已解除[周期发布 `RecurringDrawer` 仍单场次/次]；**服务保障** 12 选 N chips 选择器[`GUARANTEE_OPTIONS` 与后端 `ServiceGuarantee`/小程序常量对齐]进发布/编辑/周期表单+详情回显；**活动负责人板块** `LeaderBoard`[详情内：负责人名单+移除+现场管理 开始/结束/统一签退，渲染门槛 `activity:manage`||`activity:leader-assign`，后端 `GET /a/activity/activities/{id}/leaders` 也已 `SaMode.OR` 双权限]+`LeaderPicker` 两页签选人[从报名志愿者（`activity:enroll-view`）/从管理团队（`GET /a/user/volunteers?managerFlag=1`，`user:list`），无权时显式提示所缺权限而非静默空列表；指派一律发 `leaderType=1`+volunteer.id]）；**活动发布审核 / 报名管理 / 服务记录·积分·考勤变更·补录审核 / 组织(小组·分队) / 子账号·权限·志愿者标记授权**（M2 全部完成）；**志愿者管理**（M3，`/a/user/volunteers`：列表/详情/修改[仅超管]/停用·恢复/删除/导出；状态约定 `0正常/1禁用/2注销` 已与 `MAPS.acct` 翻正对齐、注销态视终态不提供前端反注销；**重置密码=清空密码**（V20 后启用，「更多」菜单按 `user:pwd-reset` **且仅正常态 status=0** 显隐——禁用/注销态无法用「手机号验证码重登」完成自设故不展示，确认弹窗说明清空后需手机号验证码重登再自设）；菜单 `user:list` 显隐、操作按 `user:status`/`user:delete`/`user:export`/`user:pwd-reset` 及超管显隐；mock `data.js` 权限码同步真实 `user:*`[该文件后已删，见 M5]）；**概览数据看板**（M4，`page-overview.js`：头部 6 张统计卡拉 `GET /a/data/dashboard`；7 张待办卡走各权限受控端点 `size=1` 取 total——仅对当前账号<b>有该端点权限</b>的项发请求防 403，含新全局报名 `GET /a/activity/enrollments?status=0`；计数端点+countPerm 收敛到 `shell.js` 的 `TODO_SOURCES`，侧边栏真实待办角标与概览待办卡共用）。
- **M5 生产硬化（前端部分已完成）**：去运行期 Babel 与 unpkg CDN（本地 `assets/vendor/` React 生产版，全部普通 `<script>`）；脚本统一 `.js` 后缀（防 nginx `nosniff` 下 `.jsx` MIME 拒载）；`data.js` 已删——`hasPerm` 拆到 `assets/authz.js`、预览身份拆到 `assets/preview-identities.js`；入口仅 `index.html`（旧 `恒德志愿者后台.html` 已删）。**nginx 生产配置已建**：`部署/nginx.conf`（复制到 `/etc/nginx/conf.d/`；静态根 `/opt/hengde/admin-web` 只放 index.html+assets/、`/api` 反代 `127.0.0.1:8080`、nosniff/XFO/Referrer 安全头、no-cache+ETag 协商缓存[文件名无指纹不长缓存]、gzip、封 `.md`/`uploads/`/接口文档、`client_max_body_size 16m`——与 api `application.yaml` 新增 multipart 12MB/15MB、FileValidator 10MB 三层对齐[Spring 默认 1MB 会先拦掉大图，已修]；经 Docker nginx:alpine 实测 MIME/安全头/gzip/deny/304/反代全过）。**部署文档与制品已建**：`部署/`（`nginx.conf`+`hengde-api.service`+`api.env.example` 环境变量模板[全部变量+守卫必填项注释]）+ `文档/v1/部署说明.md`（构建→systemd→nginx→HTTPS→冒烟→升级回滚→白标多实例）；`上线checklist.md` 已同步（守卫校验全集[dev-login/协议正文]、admin-web 部署项、doc.html 生产 403 为预期）。**M5 剩余**：生产环境变量填真值（用户持有密钥）、上线冒烟（须真实服务器）。
...
| 路径 | 状态 |
|---|---|
| `/v/auth/login/wechat`、`/v/auth/login/dev`（开发登录，dev 限定）、`/v/auth/login/sms`、`/v/auth/login/password`、`/v/auth/password/reset`、`/v/auth/agreement`、`/v/auth/sms/codes`、`/v/auth/wechat/group-membership` | 公开 |
| `/v/**` 其余（含 `/v/auth/register`、`/v/auth/logout`） | 需登录 + **账号状态校验**（`checkVolunteerEnabled`：每次查 volunteer status，非 NORMAL（禁用/注销/已删）即登出 403，与管理端 `checkAdminEnabled` 同口径；游客 NORMAL 照常放行；兜底「停用但 token 仍在」越权窗口，覆盖改资料/换绑手机号/上传等本人写接口） |
...
- `文档/恒德志愿者平台-已完成功能与架构设计说明.docx` — **对外交付说明文档**（2026-07-03 生成）：已完成功能清单 + 架构层次 + 设计模式详解，宋体小四正式排版，面向需求方/第三方阅读；内容快照自本文件「已完成」表，重大交付后应重新生成。
