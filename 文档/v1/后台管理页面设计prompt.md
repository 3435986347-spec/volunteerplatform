...
- **权限驱动 UI（核心）**：**菜单项与操作按钮按当前账号权限点显隐**；无权限的菜单不渲染、无权限的按钮隐藏或禁用。超管拥有全部。每个接口下方标注了它要求的权限点（`@SaCheckPermission`），按此控制对应按钮。标「仅超管」的按钮只在超级管理员可见。
  - ✅ **登录后调 `GET /a/auth/me`** 拿当前账号身份与权限码：返回 `{ adminId, username, realName, department, superAdmin, permissionCodes:[] }`（超管 `permissionCodes` 为 `["*"]`、`superAdmin=true`）。整套菜单/按钮显隐据此渲染；登录态即可、无需额外权限点。**此接口已实现**，不必再 mock。
- **导出类**接口（Excel）走浏览器下载（返回文件流，不是 JSON），点击后触发下载。
- **上传约定**：✅ 通用上传 `POST /a/files/upload`（`multipart/form-data`，字段名 `file` + **`dir` 必传**）**已实现**——返回 `{ url, name, size }`，把 `url` 填进对应表单字段即可。**按 `dir` 做权限 + 类型双门槛**（防低权限账号乱传公开文件）：

  | dir | 需要的权限点 | 允许类型 | 用途 |
  |---|---|---|---|
  | banner | pub:banner | 仅图片 | 轮播图 |
  | announcement | pub:announcement | 仅图片 | 公告封面/插图 |
  | activity | activity:publish 或 edit | 仅图片 | 活动封面 |
  | summary | activity:manage | 仅图片 | 活动总结图 |
  | file | pub:file | 图片+文档(pdf/doc/xls…) | 文件下载板块 |

  未知/缺失 `dir` 一律拒绝（不静默归类）。传图流程：上传 → 裁剪 → 把成品再调本接口拿 `url`。另：小组批量导入是 `POST /a/organization/groups/import`（multipart Excel）。OSS 未配置时返回占位 URL，本地联调照跑。
...
**视角 B · 部门子账号示例（「组织部」，分配 `activity:enroll-view`、`activity:enroll-audit`、`org:group-audit`、`org:squad-audit`）**
- 侧边栏**只剩**：报名管理（看名单 + 审核）、志愿小组—建组审批、归属分队—加入审批。
...
- 进「归属分队」页：加入申请列表走**全局**待审接口 `GET /squads/applications`（`org:squad-audit`，每行带 squadName）+ 「批准 / 拒绝」（按 applicationId，全局列表已提供）；看不到「建/改/删分队」（属 `org:squad-manage`）。**注**：若要看某个具体分队的全部申请历史则需 `squad-manage` 才能定位分队，但日常加入审批用全局接口、单 `squad-audit` 即可闭合。

> **配权口诀（务必体现在示例里）**：**「审核权」往往要搭一个「查看/列表权」才闭合**——只给 audit 不给对应的列表权，会出现「有按钮、无数据入口」。典型：报名审核要配 `enroll-view`。建组审核（`/groups/applications`）与分队加入审核（`/squads/applications`）都有全局待审列表，单 `group-audit`／`squad-audit` 即自洽。
...
■ 活动管理
    ...
    · 活动发布审核            (activity:publish-audit)  ← 审小程序提交的活动
■ 组织管理
...
- **登录后必调**：当前管理员资料 + 权限码 `GET /a/auth/me` → `{ adminId, username, realName, department, superAdmin, permissionCodes:[] }`（驱动菜单/按钮显隐与顶栏/水印；登录即可）
- 通用上传（传图/文件拿 URL）：`POST /a/files/upload`（multipart `file` + **必传** `dir`，按 dir 校验写权限+类型）→ `{ url, name, size }`
...
- **全局**待审加入申请（不按分队，概览/统一审批用，默认 status=0 可传覆盖，每行带 squadName）：`GET /a/organization/squads/applications` `[org:squad-audit]`
- 某分队加入申请列表：`GET /a/organization/squads/{id}/applications` `[org:squad-audit]`
...
> **状态说明**：活动 `status` = `1已发布 / 2已结束 / 3已取消 / 4待审核发布 / 5发布被驳回`（0草稿保留未用）。后台 `/a` 发布的活动**直接 `status=1` 上线**；小程序提交的活动落 `status=4`（见下「活动发布审核」）。

### 7b. 活动发布审核（小程序提交的活动需后台审核，后台直发不审）
**背景**：管理团队志愿者在小程序 `POST /v/activity/activities`（需 `activity:publish`）提交的活动**不直接上线**，落「待审核发布」(`status=4`)、志愿者端不可见；**后台直接发布的不进此队列**。审核是独立菜单/待办。
- 审核列表（带提交人姓名；`status` 默认 4 待审核，传 5 看**已驳回**记录）：`GET /a/activity/activities/pending-reviews` `[activity:publish-audit]`
- 待审/驳回活动**完整详情**（含驳回原因/审核人/时间；审核者看全字段决定通过/驳回，无需 `activity:menu`）：`GET /a/activity/activities/{id}/review-detail` `[activity:publish-audit]`
- 审核通过（活动上线 `status→1`）：`POST /a/activity/activities/{id}/publish-approve` `[activity:publish-audit]`
- 审核驳回（`status→5`，body 可填 `reason`）：`POST /a/activity/activities/{id}/publish-reject` `[activity:publish-audit]`
可视化：审核队列(提交人/活动名/时间/地点，可切「待审/已驳回」tab) + 「查看完整详情（走 `…/{id}/review-detail`，含驳回原因/审核留痕）→ 通过 / 驳回(填原因)」审核流；概览可加「待审发布活动」待办卡片。**权限边界（已闭合）**：① 常规活动列表 `GET /a/activity/activities` 与详情 `GET …/{id}`（`activity:menu`）**默认排除待审(4)/驳回(5)**；② **所有按 id 的常规管理写动作对待审/驳回活动一律拒绝**——修改/删除/复制活动、指派/取消/查看负责人、活动补录都拒（否则有相应权限者知道 id 就能绕开审核改动/复制上线、或给未上线活动落服务记录与积分）。审核域活动只能经 publish-approve/reject 处置；报名/签到等本就要求活动已发布，天然不受影响。`activity:publish-audit` 是 V19 新增点，只发后台审核人（部长），不开放给志愿者。
