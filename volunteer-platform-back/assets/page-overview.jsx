...
function OverviewPage(props) {
  ...
  var cards = [
    ...
    { key: 'attendance', label: '待审考勤变更', icon: 'swap', color: '#2f54eb', target: 'attendance', code: ['activity:attendance-edit', 'activity:attendance-audit'],
      value: HD.ATTENDANCE_CHANGES.filter(function (r) { return r.status === 0; }).length,
      source: 'GET /a/activity/attendance-changes?status=0 ✓' },
    { key: 'backfill', label: '待补录审核', icon: 'history', color: '#eb2f96', target: 'backfill', code: ['activity:backfill', 'activity:backfill-audit'],
